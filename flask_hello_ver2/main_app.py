...
@app.route('/tmt/start', methods=['POST'])
def tmt_start():
    session['start_time'] = time.time()
    session['completion_time'] = None
    session['errors'] = 0
    ...
    return jsonify({'status': 'success', 'total_numbers': len(NUMBERS_TMT)})

@app.route('/tmt/record_click', methods=['POST'])
def tmt_record_click():
    try:
        data = request.get_json()
        ...
        if is_correct:
            ...
            if current_number >= len(NUMBERS_TMT):
...
@app.route('/tmt/complete', methods=['POST'])
def tmt_complete():
    try:
        session['completion_time'] = time.time()
